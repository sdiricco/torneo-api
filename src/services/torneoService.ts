...
import { scrapeTableFromAICSWebSite, scrapeHRefsFrommAICSWebSite, scrapeHRefsTeamsFrommAICSWebSite, scrapeHRefsTournaments } from "../helpers/scraper";
import * as objectUtils from "../helpers/object";
...
const getTeamDetailsUrl = (id: string): string => `${baseUrl}/vedisquadre.php?=${id}`;
const getTournamentHomeUrl = (id: string): string => `${baseUrl}/homegirone.php?id=${id}`;
const getDisciplinaryMeasurementsUrl = (id: string): string => `${baseUrl}/provv.php?id_girone=${id}`;
const getPlayersRankingRankingUrl = (id: string): string => `${baseUrl}/marcatori.php?id_girone=${id}`;
const getCalendarHomeUrl = (id: string): string => `${baseUrl}/calendario.php?id_girone=${id}`;
const getCalendarPageUrl = (id: string, week: number | null): string => `${baseUrl}/calendario.php?id_girone=${id}&n_giornata=${week || ""}`;
...
const getTeamsRanking = async (id: string): Promise<any[]> => {
  try {
    ...
    const decodedTable: any[] = decodeTable(rawTable, AICS_TEAMS_RANKING_KEY_MAPPING);
...
async function getLatestMatchResults(id: string): Promise<any[]> {
  try {
    ...
    const decodedTable: any[] = decodeTable(rawTable, AICS_LAST_RESULTS_KEY_MAPPING);
...
async function getNextMatches(id: string): Promise<any[]> {
  try {
    ...
    const decodedTable: any[] = decodeTable(rawTable, AICS_NEXT_MATCHES_KEY_MAPPING);
...
const getPlayersStats = async (id: string): Promise<any[]> => {
  try {
    ...
    const decodedPlayersRankingTable: any[] = decodeTable(rawPlayersRankingTable, AICS_PLAYERS_RANKING_KEY_MAPPING);
    ...
    const rawSpecialMeasuresTable: any[] = await scrapeTableFromAICSWebSite(url, 2);
    const decodedSuspansionsTable: any[] = decodeTable(rawSuspansionsTable, AICS_SUSPANSIONS_KEY_MAPPING);
    const decodedWarningsTable: any[] = decodeTable(rawWarningsTable, AICS_WARNINGS_KEY_MAPPING);
    const decodedSpecialMeasuresTable: any[] = decodeTable(rawSpecialMeasuresTable, AICS_SPECIAL_MEASURES_KEY_MAPPING);
...
const getTournamentCalendar = async (id: string, week: string | null = null): Promise<any> => {
  try {
    ...
    const decodedTable: any[] = decodeTable(rawTable, AICS_MATCH_RESULTS_KEY_MAPPING);
    const formattedTable: any[] = formatMatchResults(decodedTable);
    return {
...
function formatMatchResults(decodedTable: any[] = []): any[] {
  return decodedTable.map((item) => {
    const dateString: string = `${item.date} ${item.time}`;
    const date: any = DateTime.fromFormat(dateString, "dd/MM/yyyy HH:mm", {
      zone: "Europe/Rome",
    });
...
function formatNextMatchesResults(decodedTable: any[] = []): any[] {
  return decodedTable.map((item) => {
    const dateString: string = `${item.date} ${item.time}`;
    const date: any = DateTime.fromFormat(dateString, "dd/MM/yyyy HH:mm", {
      zone: "Europe/Rome",
...
function decodeTable(rawTable: any[], translation: any): any[] {
  return rawTable.map((obj) => objectUtils.removeSpacesFromKeys(obj)).map((obj) => objectUtils.decodeKeys(translation, obj));
}

function splitScore(score: string = ""): number[] {
  return score.split("-").map(Number);
}

export { getTeams, getTeamDetails, getTournaments, getTournamentDetails, getTeamsRanking, getLatestMatchResults, getPlayersStats, getTournamentCalendar };
